  ...
  return (
    <>
      ...
      <section className="panel wizard" aria-live="polite">
        ...
        {step === 7 && <><h2>Виконання</h2>{accepted ? <><p>Завдання <code>{accepted.jobId}</code></p><p>Стан: <StatusBadge value={job.data?.status ?? 'queued'} /></p>{job.data && <progress max="100" value={job.data.progressPct} aria-label="Прогрес імпорту" />}{job.error && <ErrorState error={job.error} retry={() => void job.refetch()} />}{importResult.data && <div className="success-box"><strong>Імпорт завершено.</strong><p>Версія: <code>{importResult.data.datasetVersionId}</code></p><div className="inline-actions"><Link className="button" to={`/dataset-versions/${importResult.data.datasetVersionId}/quality`}>Перевірити якість</Link><Link className="button" to={`/dataset-versions/${importResult.data.datasetVersionId}/analysis`}>Відкрити аналіз</Link></div></div>}</> : <p>Очікуємо запуск.</p>}</>}
