...
export function ImportWizardPage() {
  ...
  const { control, register, getValues, formState: { errors }, setError } = useForm<FormValues>({ defaultValues: { datasetName: 'Новий набір', profile: 'uci', delimiter: ',', timestampColumn: 'timestamp', valueColumn: 'energy_kwh', targetSemantic: 'energy', unit: 'kwh', timezone: 'Europe/Paris', duplicatePolicy: 'reject' } })
  ...
  return (
    <>
      ...
      <section className="panel wizard" aria-live="polite">
        ...
        {step === 6 && <><h2>Підтвердження</h2><dl className="details-grid"><div><dt>Профіль</dt><dd>{profile}</dd></div><div><dt>Файл</dt><dd>{file?.name ?? '—'}</dd></div><div><dt>Часовий пояс</dt><dd>{timezone}</dd></div><div><dt>Дублікати</dt><dd>{duplicatePolicy}</dd></div></dl>{submit.error && <ErrorState error={submit.error} />}<button className="button primary" type="button" onClick={() => submit.mutate()} disabled={submit.isPending}>{submit.isPending ? 'Ставимо в чергу…' : 'Запустити імпорт'}</button></>}
