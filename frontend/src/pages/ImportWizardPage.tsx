...
export function ImportWizardPage() {
  ...
  const { register, watch, getValues, formState: { errors }, setError } = useForm<FormValues>({ defaultValues: { datasetName: 'Новий набір', profile: 'uci', delimiter: ',', timestampColumn: 'timestamp', valueColumn: 'energy_kwh', targetSemantic: 'energy', unit: 'kwh', timezone: 'Europe/Paris', duplicatePolicy: 'reject' } })
  const values = watch()
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      const result = await api.imports.createDatasetImport({
        ...
        targetSemantic: parsed.data.targetSemantic,
  ...
  return (
    <>
      <PageHeader title="Майстер імпорту" description="Створіть незмінну сирувату версію даних із відтворюваними параметрами імпорту." />
      ...
      <section className="panel wizard" aria-live="polite">
        ...
        {step === 6 && <><h2>Підтвердження</h2><dl className="details-grid"><div><dt>Профіль</dt><dd>{values.profile}</dd></div><div><dt>Файл</dt><dd>{file?.name ?? '—'}</dd></div><div><dt>Часовий пояс</dt><dd>{values.timezone}</dd></div><div><dt>Дублікати</dt><dd>{values.duplicatePolicy}</dd></div></dl>{submit.error && <ErrorState error={submit.error} />}<button className="button primary" type="button" onClick={() => submit.mutate()} disabled={submit.isPending}>{submit.isPending ? 'Ставимо в чергу…' : 'Запустити імпорт'}</button></>}
        {step === 7 && <><h2>Виконання</h2>{accepted ? <><p>Завдання <code>{accepted.jobId}</code></p><p>Стан: <StatusBadge value={job.data?.status ?? 'queued'} /></p>{job.data && 'progressPercent' in job.data && <progress max="100" value={Number(job.data.progressPercent ?? 0)} aria-label="Прогрес імпорту" />}{job.error && <ErrorState error={job.error} retry={() => void job.refetch()} />}{importResult.data && <div className="success-box"><strong>Імпорт завершено.</strong><p>Версія: <code>{importResult.data.datasetVersionId}</code></p><div className="inline-actions"><Link className="button" to={`/dataset-versions/${importResult.data.datasetVersionId}/quality`}>Перевірити якість</Link><Link className="button" to={`/dataset-versions/${importResult.data.datasetVersionId}/analysis`}>Відкрити аналіз</Link></div></div>}</> : <p>Очікуємо запуск.</p>}</>}
