...
export function AnalysisPage() {
  ...
  const seriesOption = useMemo(() => ({ tooltip: { trigger: 'axis' }, xAxis: { type: 'category', data: series.data?.points.map((point) => point.timestamp.toLocaleString('uk-UA')) ?? [], axisLabel: { hideOverlap: true } }, yAxis: { type: 'value', name: 'кВт·год' }, dataZoom: [{ type: 'inside' }, { type: 'slider' }], series: [{ name: 'Споживання', type: 'line', showSymbol: false, data: series.data?.points.map((point) => point.energyKwh) ?? [] }] }), [series.data])
  const hourlyOption = useMemo(() => ({ xAxis: { type: 'category', data: hourly.data?.points.map((point) => point.label) ?? [] }, yAxis: { type: 'value', name: 'кВт·год' }, tooltip: { trigger: 'axis' }, series: [{ type: 'bar', name: 'Середнє', data: hourly.data?.points.map((point) => point.meanEnergyKwh) ?? [] }] }), [hourly.data])
  const weekdayOption = useMemo(() => ({ xAxis: { type: 'category', data: weekday.data?.points.map((point) => point.label) ?? [] }, yAxis: { type: 'value', name: 'кВт·год' }, tooltip: { trigger: 'axis' }, series: [{ type: 'bar', name: 'Середнє', data: weekday.data?.points.map((point) => point.meanEnergyKwh) ?? [] }] }), [weekday.data])
  const heatmapOption = useMemo(() => ({ tooltip: {}, xAxis: { type: 'category', data: Array.from({ length: 24 }, (_, index) => String(index)) }, yAxis: { type: 'category', data: weekdays }, visualMap: { min: 0, max: Math.max(1, ...(heatmap.data?.points.map((point) => point.meanEnergyKwh) ?? [1])), calculable: true, orient: 'horizontal' }, series: [{ type: 'heatmap', data: heatmap.data?.points.map((point) => [point.hour, point.isoWeekday - 1, point.meanEnergyKwh]) ?? [] }] }), [heatmap.data])
  const distributionOption = useMemo(() => ({ xAxis: { type: 'category', data: distribution.data?.bins.map((bin) => `${bin.lowerKwh.toFixed(2)}–${bin.upperKwh.toFixed(2)}`) ?? [], axisLabel: { rotate: 35 } }, yAxis: { type: 'value', name: 'Кількість' }, tooltip: { trigger: 'axis' }, series: [{ type: 'bar', data: distribution.data?.bins.map((bin) => bin.sampleCount) ?? [] }] }), [distribution.data])
