...
import { DuplicatePolicy, JobStatus, type QualityIssueResponse } from '../generated/api/models'
...
function analysisUrl(targetVersionId: string, summary: Record<string, unknown>, items: QualityIssueResponse[]) {
  const base = `/dataset-versions/${targetVersionId}/analysis`
  const totalRows = summaryValue(summary, ['total_rows'])
  if (!totalRows) return base
  const fullRangeIssue = items.find((issue) => issue.occurrenceCount === totalRows && issue.rangeStart && issue.rangeEnd)
  if (!fullRangeIssue?.rangeStart || !fullRangeIssue.rangeEnd) return base
  const endExclusive = new Date(fullRangeIssue.rangeEnd)
  endExclusive.setUTCDate(endExclusive.getUTCDate() + 1)
  const from = fullRangeIssue.rangeStart.toISOString().slice(0, 10)
...
export function DataQualityPage() {
  ...
  return (
    <>
      ...
      <section className="panel"><h2>Погодинна версія</h2><p>Основна політика: покриття щонайменше 90%, коротка інтерполяція до 5 хвилин і явне відхилення конфліктних дублікатів. Після успішного завершення система автоматично відкриє аналіз нової погодинної версії.</p>{transform.error && <ErrorState error={transform.error} />}{!transform.data ? <button className="button primary" type="button" onClick={() => transform.mutate()} disabled={transform.isPending}>{transform.isPending ? 'Ставимо в чергу…' : 'Підготувати погодинну версію'}</button> : transformationFailed ? <div className="state-card error" role="alert"><h3>Не вдалося створити погодинну версію</h3><p>{job.data?.errorDetail ?? `Завдання завершилося зі станом ${transformationStatus}.`}</p><p className="muted">Завдання: <code>{transform.data.jobId}</code></p>{retry.error && <p>{retry.error instanceof Error ? retry.error.message : 'Не вдалося повторити завдання.'}</p>}<button className="button primary" type="button" onClick={() => retry.mutate()} disabled={retry.isPending}>{retry.isPending ? 'Повторюємо…' : 'Повторити перетворення'}</button></div> : <div className="success-box" role="status" aria-live="polite"><strong>Готуємо погодинну версію…</strong><p>Стан: <StatusBadge value={transformationStatus ?? 'queued'} /> · прогрес {job.data?.progressPct ?? 0}%</p><progress max="100" value={job.data?.progressPct ?? 0} aria-label="Прогрес підготовки погодинної версії" /><p className="muted">Завдання: <code>{transform.data.jobId}</code> · цільова версія: <code>{transform.data.targetVersionId}</code>.</p><p className="muted">Після завершення перехід до її аналізу відбудеться автоматично.</p>{job.error && <ErrorState error={job.error} retry={() => void job.refetch()} />}</div>}</section>
    </>
  )
}
