...
| ADR-024 | Artifact-backed bounded exports | generated bytes get checksums; only export-purpose artifacts stream through safe downloads |
| ADR-025 | Hardened Compose edge deployment | explicit migration gate, private backend network, non-root read-only app containers and same-origin Nginx proxy |
